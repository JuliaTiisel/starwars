...
public class MenuState implements State {
	...
	private String[] options = { "START", "ARMORY", "EXIT" };
	private Font font1;
	private Font font2;
	...
	public void render(Graphics g) {
		...
		g.setFont(font1);
		g.drawString("STAR WARS", Game.WIDHT / 2 - g.getFontMetrics().stringWidth("STAR WARS") / 2,
				Game.HEIGHT * 1 / 4);
		...
		for (int i = 0; i < options.length; i++) {
			...
			g.drawString(options[i], Game.WIDHT / 2 - g.getFontMetrics().stringWidth(options[i]) / 2,
